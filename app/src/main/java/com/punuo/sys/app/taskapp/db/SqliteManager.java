...
public class SqliteManager {
    ...
    public void insertInfo(Context context, TaskInfo info) /*把TaskInfo类中的数据插入数据库中。第一个参数为context，第二个参数为要插入的工单类的实例*/ {
    ...
    public List<TaskInfo> getInfo(Context context, int state) /*从数据库中获得新工单，已完成工单和未完成工单。第一个参数为context，第二个参数为0，1，2分别表示取出新工单，未完成工单和已完成工单，返回值为选定类型的所有工单，以list形式返回*/ {
        ...
        while (cursor.moveToNext()) {
            TaskInfo infoTest = new TaskInfo();
    ...
    public List<TaskInfo> getRowInfo(String dev_id, String receive_time, Context context) /*从数据库中取出单个工单项目，第一个参数为要取出工单的dev_id，第二个参数为要取出工单的receive_time，第三个参数为context，返回值为包含整个工单的list*/ {
        List<TaskInfo> info = new ArrayList<TaskInfo>();
        db = new MyDatabaseHelper(context, "dict2", null, 1);
        sDatabase = db.getWritableDatabase();
